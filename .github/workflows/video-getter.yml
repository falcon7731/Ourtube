name: 02 - 🎬 دانلود یوتیوب

on:
  workflow_dispatch:
    inputs:
      video_urls:
        description: '🔗 لینک ویدیو ( چند لینک با فاصله )'
        required: true
        type: string
      download_subtitles:
        description: '📝 زیرنویس ( فارسی و انگلیسی )'
        required: false
        default: false
        type: boolean
      quality:
        description: '🎬 کیفیت و نوع'
        required: false
        default: 'best'
        type: choice
        options:
          ...
          - audio
      password:
        description: '🔒 رمز ( اختیاری برای محافظت )'
        required: false
        default: ''
        type: string
...
jobs:
  get-video:
    ...
    steps:
      ...
      - name: آماده سازی گیت برای فایلهای بزرگ
        run: |
      ...
      - name: نصب پیش نیازها
        run: |
          ...
          # Install Deno (JS runtime required by yt-dlp for Ourtube n-challenge solving)
      ...
      - name: دانلود تنظیمات
        env:
          YT_URLS: ${{ github.event.inputs.video_urls }}
            ...
            for METHOD in 1 2 3 4 5 6 7 8; do
              ...
              case $METHOD in
                ...
                2)
                  ...
                  ;;
                3)
                  yt-dlp --proxy "socks5://127.0.0.1:1080" --extractor-args "youtube:player_client=web,mweb,android_vr" --js-runtimes deno --remote-components ejs:github $SFLAGS $COMMON_SUB "$SURL" 2>&1 || true
                  ;;
                4)
                  yt-dlp --proxy "socks5://127.0.0.1:1080" --extractor-args "youtube:player_client=mweb" $SFLAGS $COMMON_SUB "$SURL" 2>&1 || true
                  ;;
                5)
                  yt-dlp --proxy "socks5://127.0.0.1:1080" --extractor-args "youtube:player_client=android_vr" $SFLAGS $COMMON_SUB "$SURL" 2>&1 || true
                  ;;
                6)
                  yt-dlp --extractor-args "youtube:player_client=web" --js-runtimes deno --remote-components ejs:github $SFLAGS $COMMON_SUB "$SURL" 2>&1 || true
                  ;;
                7)
                  yt-dlp --extractor-args "youtube:player_client=mweb" $SFLAGS $COMMON_SUB "$SURL" 2>&1 || true
                  ;;
                8)
                  yt-dlp --proxy "socks5://127.0.0.1:1080" --extractor-args "youtube:player_client=android" --user-agent "Mozilla/5.0 (Linux; Android 12; SM-S906N Build/QP1A.190711.020) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Mobile Safari/537.36" $SFLAGS $COMMON_SUB "$SURL" 2>&1 || true
                  ;;
              esac
      ...
      - name: متوقف کردن کانتینر
        if: always()
        run: |
          docker stop warp || true
          docker rm warp || true

      - name: ذخیره سازی دانلود شده ها
        run: |
